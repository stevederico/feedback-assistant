...
import { readFile, readFileSync, mkdir, stat } from 'node:fs';
 ...
 *
 * @param value - Unknown value parsed from config.json
 * @returns True if the value has the expected staticDir/database shape
 */
...
// Skip on the public widget route so the script can be loaded from any customer
// origin — secureHeaders sets CORP=same-origin by default, which Chrome enforces
// (ERR_BLOCKED_BY_RESPONSE.NotSameOrigin).
const secureHeadersHandler = secureHeaders(__testBuildSecureHeadersOptions());
app.use('*', async (c, next) => {
...
app.post("/api/signup", async (c) => {
  try {
    ...
    try {
      ...
      // rawDb is set during boot (the schema bootstrap throws if it isn't), so
      // it is always present here — this guard also narrows the nullable type.
      if (!rawDb) {
      ...
      sqliteDb.exec('BEGIN');
      try {
        sqliteDb.prepare('INSERT INTO Orgs (id, name, created_at) VALUES (?, ?, ?)').run(orgId, orgName, now);
        await db.insertUser({ _id: insertID, email, name, created_at: now });
        sqliteDb.prepare('UPDATE Users SET org_id = ? WHERE _id = ?').run(orgId, insertID);
        await db.insertAuth({ email, password: hash, userID: insertID });
        sqliteDb.exec('COMMIT');
      } catch (txErr) {
        // Roll the whole Org + User + Auth transaction back so a failed signup
        // never leaves an orphaned org or user record behind.
        try { sqliteDb.exec('ROLLBACK'); } catch { /* ignore */ }
        logger.error('Signup transaction failed, rolled back', { error: errorMessage(txErr) });
        throw txErr;
      }
...
// The feedback tables are SQLite-only, so the bound handle is a node:sqlite
// DatabaseSync — narrow databaseManager's generic `database` (unknown) to it.
...
try {
  ...
  rawDb = database as DatabaseSync;
  bootstrapFeedbackSchema(rawDb);
  const uploadsDir = await ensureUploadsDir();
  logger.info('Feedback Assistant schema ready', { uploadsDir });
} catch (e) {
  logger.error('Failed to bootstrap feedback schema', { error: (e as Error).message });
  throw e;
}
...
try {
  ...
  logger.warn('Widget bundle missing at boot — SRI integrity unavailable', { error: (e as Error).message });
}
